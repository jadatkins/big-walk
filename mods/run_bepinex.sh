...
# 2. Via config: edit the options below and run ./run_bepinex.sh without any arguments
...
# BepInEx's bundled dotnet/libcoreclr.dylib and libdobby.dylib are x86_64-only,
# so the game must run under Rosetta for them to load.
...
case ${os_type} in
    ...
    Darwin*)
        ...
        # Capture native diagnostics as well as the managed BepInEx log.
        exec arch \
